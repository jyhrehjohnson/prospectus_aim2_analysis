...
│ - Combine: D_combined = α×D_mahal + (1-α)×D_gower        │
└─────────────────────────────────────────────────────────┘
                          ↓
┌─────────────────────────────────────────────────────────┐
...
## Function 1: Calculate Combined Distance
...
### How It Works:
```{r combined-distance-explanation}
...
# D_combined = α × D_mahalanobis + (1-α) × D_gower
...
### STEP 3: Calculate Combined Distance
...
### Methods Section:
```
"Species delimitation was performed using a combined distance metric 
...
discrete morphological characters). The combined distance (D_combined = α × D_Mahalanobis + (1-α) × D_Gower, α = 0.65) was calculated for 
...
- GitHub: github.com/yourusername/combined-distance
...
# Write the Rmd file
writeLines(methods_rmd, "Aim 2_Pipeline.Rmd")

cat("R Markdown file created: Aim 2_Pipeline.Rmd\n")
cat("Compiling to PDF...\n\n")

# Compile to PDF
tryCatch({
  rmarkdown::render("Aim 2_Pipeline.Rmd", 
                   output_format = "pdf_document",
                   output_file = "Aim 2_Pipeline.pdf")
  
  cat("\n========================================\n")
  cat("SUCCESS! PDF CREATED\n")
  cat("========================================\n\n")
  cat("File saved as: Aim 2_Pipeline.pdf\n")
  cat("Location: ", getwd(), "\n\n")
  ...
  cat("You can:\n")
  cat("1. Open Aim 2_Pipeline.Rmd in RStudio and click 'Knit'\n")
